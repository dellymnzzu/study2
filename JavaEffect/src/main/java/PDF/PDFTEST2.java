...
//font set
        ...
        headerFont = PdfFontFactory.createFont("나눔손글씨 나는 이겨낸다.ttf","Identity-H");
        bodyFont = PdfFontFactory.createFont("나눔손글씨 나는 이겨낸다.ttf","Identity-H");
// 실수 배열 생성 - > 테이블 생성
        ...
        //call 1번 
            ...
            Cell publisherDateCell = new Cell().add(new Paragraph(publishedDate)).setFont(bodyFont);
            table.addCell(publisherDateCell);
    ...
    private List<Map<String, String>> createDummyData() {
        List<Map<String, String>> books = new ArrayList<>(); // 어레이리스트 객체 생성 -> MAP -> <String,String>
        Scanner scanner = new Scanner(System.in);
        System.out.println("책 개수를 입력하세요.");
        int bookCount = scanner.nextInt(); //숫자입력
        scanner.nextLine(); // 개행문자 제거
        for (int i = 1; i <= bookCount; i++) {
            Map<String, String> book = new HashMap<>(); // map 객체를 생성
            System.out.printf("\n [%d번재 책 정보 입력]\n", i);
            System.out.print("제목 : ");
            book.put("title",scanner.nextLine());//맵에 추가
            System.out.print("저자 : ");
            book.put("authors",scanner.nextLine());//맵에 추가
            System.out.print("출판사 : ");
            book.put("publisher",scanner.nextLine());//맵에 추가
            System.out.print("출판일(YYYY-MM-DD) : ");
            book.put("publishedDate",scanner.nextLine());//맵에 추가
            System.out.print("썸네일 URL : ");
            book.put("thumbnail",scanner.nextLine());//맵에 추가
