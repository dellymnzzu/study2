...
class Controller1{// 전화번호 컨트롤을 하는 class
    ...
    void delete() throws IOException {
        ...
        //index가 -1면 실행
        if(index == -1) {  // -1로 하는 이유 : 인덱스 번호를 사용하기 때문에 번호가 없는 -1로 하는게 좋다.
            System.out.println("삭제할 이름이 없습니다.");
        }//위의 조건이 아니면 실행
        else {
            //FileWriter객체를 생성
            //생성자 매개변수 -> 파일 경로, false(덮어쓰기)
            FileWriter fw1 = new FileWriter("C:\\Phonenumber\\phone.txt",false);  // 덮어쓰기 되니 데이터가 삭제된다.
            //---> 파일에 내용이 초기화 됩니다.
            ...
            for(int i = 0;i<tel.size();i++) {  // 이름을 입력하면 이름이라고 하면 나온다.  이름을 나오지 않게 하려면 i를 1로 바꾸면 된다.
                //문자열 만듭니다. 이름\t\t전화번호\t\t주소\r\n
    ...
    void allPrint() throws IOException {
		...
		*/
        ...
        tel.clear();
        //파일 읽기 객체를 생성
        FileReader fr = new FileReader("C:\\Phonenumber\\phone.txt");
...
public class Q1 {
    public static void main(String[] args) throws IOException {
        ...
        if(f.exists()) {  // 존재여부 확인
            ...
            while((line = reader.readLine()) != null) {
                splitLine = line.split("\t\t");  // 특정 문자로 잘라준다.  \t전에 잘라준다.
        ...
        while(true) {
            ...
            if(c.tel.size() == 0) {  // 사이즈가 0이면
                // 덮어쓰기 객체 생성
                fw = new FileWriter("C:\\Phonenumber\\phone.txt",false); // 덮어쓰기
            }
            else { // 사이즈()가 0이 아니라면
                // 이어쓰기 객체 생성
                fw = new FileWriter("C:\\Phonenumber\\phone.txt",true); // 이어쓰기
            }
            //입력이 1인경우
            if(num == 1) {
