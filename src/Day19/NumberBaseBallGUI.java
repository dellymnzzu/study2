...
class RCom{  // 컴퓨터가 숫자를 고르는 클래스
	private int[] com;
	RCom(){
		Random r = new Random();
		com = new int[3];
 		// com의 배열의 각 숫자마다 같으면 다시 돌리고 아니면 삐져나오라는 뜻
...
class UserCheck{  // 사용자가 입력한 숫자와 비교
	int strike;
	int ball;
	boolean out;
...
class NumberBaseBallPanel extends JPanel{  // 게임 결과를 화면에 그리기
	int strike = 0;
	int ball = 0;
	boolean out = false;
...
public class NumberBaseBallGUI extends JFrame implements ActionListener{
	...
	public void actionPerformed(ActionEvent e) {
		int[] user = new int[3];  // 유저의 배열을 생성했다. (3 생성)
		String[] temp = jt.getText().split(""); // 문자열 추출 후 한글자씩 잘라서 초기화 해줬다.
		for(int i = 0;i<temp.length;i++) {  // i가 0부터 temp의 배열 수보다 작을 때까지 1씩 증가해라.
			user[i] = Integer.parseInt(temp[i]);  // user의 i번째를 정수형으로 변경해주었다.
		}
